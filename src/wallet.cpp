...
// Raindrops: optional setting to unlock wallet for block minting only;
...
// Raindrops: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Raindrops: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for(;;)
            {
                ...
                // Raindrops: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // Raindrops: not avatar mode
                        {
...
// Raindrops: create coin stake transaction
...
// Raindrops: check 'spent' consistency between wallet and txindex
// Raindrops: fix wallet spent state according to txindex
...
// Raindrops: disable transaction (only for coinstake)
